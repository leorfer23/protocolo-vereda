...
- Al confirmar el carrito, el cobro nace `pendiente` con su `vence` (`ventana_pago_min` del comercio) y con `instrucciones`: alias, titular, monto y una referencia corta para el concepto. Las instrucciones las ve únicamente el comprador, y solo mientras el pedido está activo. **Se congelan al crear el cobro**: si el comercio cambia su cuenta de cobro después, el comprador sigue viendo la cuenta a la que le dijeron que transfiera, y el cambio queda a la vista (`docs/identidad-y-verificacion.md`, cuenta de cobro).
- **Centavos únicos.** `instrucciones.monto` es lo que hay que transferir, y el nodo lo hace único entre los cobros `pendiente` por transferencia del mismo destinatario: si otro cobro de las últimas 24 h ya pidió el mismo monto a ese destinatario, descuenta centavos libres elegidos al azar (1 a 99) y lo anota en `instrucciones.ajuste_centavos` (siempre 0 o negativo: descontar es un descuento, sumar sería cobrar más que el precio exhibido). Así la transferencia se reconoce sola por el monto, sin depender del concepto que escriba la persona en su banco, que ningún banco garantiza. Los centavos los absorbe el destinatario. Un monto queda tomado hasta 24 h después de que el cobro se confirme o venza, porque una transferencia puede llegar tarde. Si no queda ninguno libre, el nodo no ajusta y el comercio distingue por `referencia`. Es el default del protocolo; el comercio lo apaga con `privado.cuenta_cobro.centavos_unicos: false`. El monto exacto también lo ve el destinatario, en `pago.monto_a_transferir` (igual a `instrucciones.monto`, sin alias ni titular), para buscarlo en su cuenta antes de confirmar: las `instrucciones` son solo de quien paga, el monto no. Si la persona transfiere de más (el monto redondo, por ejemplo), el comercio confirma como siempre y la diferencia es suya; si transfiere de menos, el cobro sigue `pendiente` y lo arreglan por el chat del pedido.
- **Referencia única.** `instrucciones.referencia` (y `pago.referencia`, que ven quien paga y el destinatario) es la segunda huella del cobro, para cuando el monto no alcanza: sin centavos únicos, o sin centavos libres. Son de 4 a 24 letras, números, espacios y guiones, para que entre en el concepto de cualquier banco; lo recomendado son 4 a 12 mayúsculas y números, más una palabra que dice el concepto cuando el mismo pedido tiene más de un cobro al mismo destinatario (`9F3A7C21`, `9F3A7C21 ENVIO`, `9F3A7C21 PROPINA`). Es única entre los cobros por transferencia del mismo destinatario que están `pendiente` o se cerraron hace menos de 24 h, con la misma ventana que los centavos. **Nunca sale del principio del id**: un UUIDv7 empieza por la hora, y todos los pedidos del mismo minuto compartirían referencia; si sale del id, del final (el azar), y si choca con otra vigente, el nodo elige otra. Qué muestra cada uno: el comprador, el monto grande con los centavos resaltados ("mandá exactamente $12.399,**63**") y la referencia para el concepto, los dos copiables con un toque; el destinatario, el mismo monto (`monto_a_transferir`) y la misma referencia, para buscarlos en su app del banco. Ninguno de los dos depende de una captura.
...
- El comercio confirma cuando ve la plata con `POST /pedidos/{id}/transferencia/confirmar`. Ahí el cobro pasa a `confirmado` y, si el pedido seguía `creado`, a `pagado`. Es el único que puede: es el único que ve su cuenta. La confirmación es una constancia firmada (abajo, "Recibido firmado y revertido").
- **Aceptar no espera a la plata.** El comercio puede aceptar, preparar y entregar un pedido con la transferencia todavía `pendiente`: el cobro queda "por confirmar" a la vista de las dos partes, y confirmarlo después vale con el pedido donde esté, sin moverlo. Aceptar es el comercio asumiendo ese riesgo; el nodo no lo decide por él.
- Si nadie confirma antes del `vence` y el pedido sigue `creado`, se aplica igual que cualquier otro pago pendiente: `vencido`, pedido `cancelado` con `pago_vencido`, se libera lo reservado. Un pedido que el comercio ya aceptó no se cancela por `pago_vencido`: el cobro queda `pendiente` para que lo confirme cuando vea la plata.
...
### Recibido firmado y revertido

"El comprador dice que pagó" y "el comercio vio la plata" son dos cosas distintas, y la segunda la firma quien la vio.

- **Avisó que transfirió.** El aviso existe solo si el comprador lo dio: `pago.transferencia` con `declarada_en`. Las apps muestran "Avisó que te transfirió · 21:14 · referencia 9F3A7C21" solo con ese campo, y siempre una línea fija: "Mirá tu app del banco, no una captura". Sin `pago.transferencia` no hay aviso.
- **Recibido.** `confirmarTransferencia` deja en `pago.confirmacion` una constancia firmada (`pago.json#/$defs/constancia_cobro`, `accion: cobro_recibido`): qué pago, qué pedido, a quién, por qué concepto, qué monto (`monto_a_transferir`) y quién lo vio, con su firma. Para los productos, la misma firma va en `pedido.firmas.cobro`. La firma es de la persona, no del comercio: la dueña, un miembro del equipo con permiso `pedidos` o el repartidor para lo suyo, así queda a nombre de quien miró la cuenta. Cualquiera la verifica con la clave pública de esa persona. El comprador ve "El local confirmó tu pago a las 21:16", con el sello Firmada.
- **Quién firma.** Con la clave en el teléfono, la manda la app. Si el nodo custodia la clave, firma el nodo con ella. Por mandato, un agente no firma por la persona: la constancia queda sin firma y con `mandato_id`, y las apps la muestran como "confirmado por su agente", sin sello. Con custodia propia y sin firma, mientras el nodo no exija la firma fresca (`docs/acceso.md`, punto 7) confirma igual y la constancia queda sin firma; cuando la exige, `422 firma_requerida`. La constancia ya es una firma de la clave sobre la operación con su instante, así que no pide además las cabeceras de firma fresca.
- **Me la desconocieron.** Si después la transferencia se va de la cuenta (el banco la devolvió, el comprador la desconoció), el destinatario la marca con `POST /pedidos/{id}/transferencia/revertir` (`revertirTransferencia`) y un `motivo`. El cobro pasa a `revertido`, que es final, con la constancia `cobro_revertido` firmada en `pago.reversion`, al lado de la confirmación. No mueve ni cancela el pedido, y nadie arbitra: quien pagó ve las dos constancias y puede contestar por el chat. Sale como `pago.revertido`, va al registro público (`docs/registro.md`) y cuenta en los hechos del comprador (`transferencias_revertidas`, `docs/hechos-del-comprador.md`).
- Vale igual para el envío y la propina que confirma el repartidor (`concepto: envio`, `propina_repartidor`).
