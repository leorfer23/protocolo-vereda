...
El nodo **no hospeda modelos**. Llama a un gateway de inferencia que el operador eligió (OpenRouter, Vercel AI Gateway u otro). La spec no nombra proveedor: solo exige medición auditable y que el costo se publique.
...
| `tope_mensual_usd` | Default y máximo que una identidad puede elegir al activar. |
...
Si no está `activa`, cualquier función responde `403 ia_no_activada`. Si el consumido del mes llega al tope, `429 tope_ia_alcanzado` con `detalle.tope_usd` y `detalle.consumido_usd` (y `Retry-After` hasta el próximo reseteo). Si el gateway no responde, `503 gateway_ia_caido`.
