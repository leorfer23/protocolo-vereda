...
| `pago.revertido` | El destinatario marcó que una transferencia que había confirmado se fue de su cuenta (`revertirTransferencia`). `datos.pago` es el cobro con `confirmacion` y `reversion`, las dos constancias firmadas. A quien pagó y al destinatario (el comercio con su equipo con `pedidos`, o el repartidor). Sale también al registro público (`docs/registro.md`) |
| `comercio.cuenta_cobro_cambiada` | Cambió el alias o el titular de `privado.cuenta_cobro` (`editarComercio`). `datos` trae `por` (la persona que la cambió), `instante` y `cuenta_cobro_desde`; nunca el alias ni el titular. A la dueña y a quien administra el comercio con permiso `datos`, en todas sus sesiones: si no la cambió él, lo ve. Sale también al registro público |
